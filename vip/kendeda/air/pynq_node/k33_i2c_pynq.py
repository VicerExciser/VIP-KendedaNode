...
""" Reference documents for the CO2Meter K-Series:
	- http://senseair.jp/wp-content/uploads/2019/01/I2C-comm-guide-2_15.pdf
	- https://co2meters.com/Documentation/Other/SE_0020_Configuration_Guide_revised8.pdf
	- https://co2meters.com/Documentation/Manuals/Manual_SE_20_25_26_27_Revised8.pdf
"""
...
def format_read_request(num_bytes, ram_address):
	...
	# request.append((ram_address >> 8) & 0xFF)  # ram_address & 0xFF00   # = (ram_address >> 8) << 8
	# request.append(ram_address & 0xFF)
	# request.append(sum(request) & 0xFF) 		## Checksum is the summation of bytes 0 + 1 + 2
	return request
...
def format_eeprom_write_request(num_bytes, eeprom_address, data):
	...
	# request = bytearray()
	request.append(EEPROM_WRITE_CMD | num_bytes)
	request.append((eeprom_address >> 8) & 0xFF)
	request.append(eeprom_address & 0xFF)
	for i in range(num_bytes):
...
def wake_sensor():
	...
	if IOP_NAME.upper() == "ARDUINO":
		# bus.open(ARDUINO_SDA, ARDUINO_SCL)
		bus = bus.open_device(0)
	else:
		bus = bus.open(PMOD_SDA, PMOD_SCL)
...
def open_bus():
	...
	if lib is not None:
		if IOP_NAME.upper() == "ARDUINO":
			# bus = lib.i2c_open(ARDUINO_SDA, ARDUINO_SCL)
